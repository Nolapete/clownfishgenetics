...
from genetics_manager.calculator_utils import fish, cross_fish_structured, analyze_results_by_recipe
...
def landing_page(request):
    ...
    for fish_recipe in all_clownfish:
        is_pure_wild_type = all(gene_pair == '+/+' for gene_pair in fish_recipe.genotype.split())
...
def get_phenotype_recipes_from_db():
    ...
    for db_recipe in CommercialPhenotypeRecipe.objects.all():
        ...
        formatted_recipes.append({"name": db_recipe.name, "criteria": criteria_function})
    formatted_recipes.append({"name": "Generic/Unnamed Hybrid", "criteria": lambda g: True})
...
def calculate_cross_htmx(request):
    if request.htmx:
        ...
        if not parent1_id or not parent2_id:
            return render(request, 'landing/partials/error_partial.html',
        ...
        results_list, total_count, all_trait_names = cross_fish_structured(parent1_fish_obj, parent2_fish_obj)

        PHENOTYPE_RECIPES_DB = get_phenotype_recipes_from_db()

        results_percentages = analyze_results_by_recipe(results_list, total_count, PHENOTYPE_RECIPES_DB,
                                                        all_trait_names)

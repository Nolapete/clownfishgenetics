...
def format_allele(p1, p2):
    """Ensures dominant (uppercase/non-wild) allele comes first in the display string."""
    if p1 == '+':
        if 'a' <= p2 <= 'z':
            return p1 + '/' + p2
        return p2 + '/' + p1
...
def cross_at_index(ind, length, h_axis, v_axis):
    """Calculates the genotype at a specific index in the virtual Punnett square table."""
    start = length // 2
    x, y = ind
    res_parts = []
    ax_ind = 0
...
def cross_fish_structured(f1, f2):
    ...
    for n in f2_type_names - f1_type_names: f1_types[n] = ('+', '+')
    for n in f1_type_names - f2_type_names: f2_types[n] = ('+', '+')

    all_trait_names = sorted(list(f1_type_names.union(f2_type_names)))
    ...
    for x in range(table_length):
        for y in range(table_length):
            genotype_list = cross_at_index((x, y), table_length, f1_final, f2_final).split()
            genotype_dict = {}
    ...
    for result_dict in results_list:
        for recipe in recipes:
            if recipe["criteria"](result_dict):
                name = recipe["name"]
                phenotype_counts[name] = phenotype_counts.get(name, 0.0) + (100. / total_count)
                break

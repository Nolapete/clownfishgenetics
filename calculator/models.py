...
class UserGenotype(models.Model):
    ...
    alleles = JSONField(default=dict)  # Stores allele pairs like {'Color': ['PP'], 'Stripe': ['Ss']}
...
class Trait(models.Model):
    ...
    inheritance_pattern = models.CharField(max_length=20, choices=INHERITANCE_CHOICES, default='dominant')
...
class GenotypePhenotype(models.Model):
    ...
    """
    trait = models.ForeignKey(Trait, on_delete=models.CASCADE, related_name='phenotypes')
...
class Clownfish(models.Model):
    ...
    genotype = models.ManyToManyField(
        ...
        through_fields=('clownfish', 'allele1') # Use the clownfish and first allele for the relation
    )
    ...
    def get_phenotype_string(self):
        ...
        phenotype_parts = [genotype.get_phenotype() for genotype in self.clownfishgenotype_set.all()]
        return ", ".join(phenotype_parts)
    
    def get_trait_genotype(self, trait_name):
        """Returns the allele names for a specific trait."""
        alleles = [g.allele.name for g in self.clownfishgenotype_set.filter(trait__name=trait_name)]
        return alleles
...
class ClownfishGenotype(models.Model):
    ...
    allele1 = models.ForeignKey(Allele, on_delete=models.CASCADE, related_name='allele1_set')
    allele2 = models.ForeignKey(Allele, on_delete=models.CASCADE, related_name='allele2_set')

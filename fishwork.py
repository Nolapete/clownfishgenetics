...
def cross_fish(f1, f2):
    ...
    """
    f2_missing = list(filter(lambda a: not in f2_type_names, f1_type_names))
    f1_missing = list(filter(lambda a: not in f1_type_names, f2_type_names))
...
def newPhen(nm, geno):
    if nm.find('ocellaris') != -1:
        if geno == 'Sf/+':
            ...
            newnm =
            fmtIt('Amphiprion ocellaris "DaVinci / Fancy White / Gladiator"')
        elif geno == 'DV/+ +/a':
            newnm =
            fmtIt('Amphiprion ocellaris "DaVinci / ' +
...
def pheno(genotype, parent1, parent2):
    ...
    p1epigra = parent1.index('Premnas sp. epigrammata')
    p2epigra = parent2.index('Premnas sp. epigrammata')
    p1darwin = parent1.index('Premnas sp. darwin')
    p2darwin = parent2.index('Premnas sp. darwin')
    parents = parent1 + parent2

    if parent1.find('ocellaris') != -1 and parent2.find('ocellaris') != -1:
        pheno = newPhen(fmtIt(p1S[0]).join([" ", fmtIt(
            p1S[1]), " (", parent1[spP1part2 + 1:], " X ",
                                            parent2[spP2part2 + 1:], ")"]), g1)
    elif p1S[0] == p2S[0]:
        pheno = fmtIt(p1S[0]).join(
            [" (", fmtIt(parent1[spP1part1 + 1:]), " X ",
             fmtIt(parent2[spP2part1 + 1:]), ")"])

    elif parent1.find('percula') != -1 and parent2.find('percula') != -1:
        pheno =
        newPhen(pheno=fmtIt(p1S[0]).join([" ",
                                          fmtIt(p1S[1]),
                                          " (", parent1[spP1part2 + 1:], " X ",
                                          parent2[spP2part2 + 1:], ")"]), g1)

    elif parent1 == parent2:
        if parents.find('Amphiprion percula "Picasso"') != -1 or
        parents.find('Amphiprion percula "Platinum"') != -1:
            if g1 == "P/+":
                ...
                pheno = fmtIt(p1S[0]).join([" ", fmtIt(p1S[1]), " (",
                                            parent1[spP1part2 + 1:], " X ",
                                            parent2[spP2part2 + 1:], ")"])
        ...
        'Premnas sp. epigrammata "Goldflake / Gold Spot / Gold Rush"':
            if g1 == "G/+":
                pheno =
                fmtIt('Premnas sp. epigrammata "Goldflake / Gold Spot /' +
                      ' Gold Rush"')
            elif g1.find("G/G") != -1:
                ...
                pheno =
                fmtIt("Premnas sp. epigrammata".join([" ",
                                                      parent1[spP1part3:]])
            ...
            pheno =
            fmtIt(parent1[0, spP1part2].join([" ", parent1[spP1part2 + 1:]]))

        else:
            pheno = fmtIt(parent1[0, spP1part1].join([" ",
                                                      parent1[spP1part1 + 1:]])

    elif parents.find('Premnas biaculeatus "Lightning"') != -1 and
    parents.find('Premnas biaculeatus "White Stripe') != -1:
        if g1 == "L/+":
            pheno = fmtIt('Premnas biaculeatus "Lightning"')
        else:
    ...
    elif parents.find('Premnas sp. epigrammata "Gold Stripe"') != -1 and
    parents.find('Premnas sp. epigrammata "Goldflake / Gold Spot / Gold Rush"')
    != -1:
        if g1 == "G/+"):
            pheno = fmtIt(
                'Premnas sp. epigrammata "Goldflake / Gold Spot / Gold Rush"')
    ...
    elif parents.find('Amphiprion "Black Snowflake/Phantom"') != -1 and
    parents.find('Amphiprion sp. darwin "Midnight"') != -1:
        ...
        pheno = fmtIt(p1S[0]).join([" ", fmtIt(p1S[1]), " (", momV, " X ",
            dadV, ")"])
        ...
        pheno = fmtIt(p1S[0]).join([" (", fmtIt("sp. epigrammata"), parent1[spP1part3), " X ",
            fmtIt(parent2[spP2part1 + 1:]), ")"])
